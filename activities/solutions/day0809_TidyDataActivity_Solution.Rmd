...
#### *Answer:*
**1.** There are `r nrow(energy_narrow)` rows and `r ncol(energy_narrow)` columns.
...
#### *Answer:*
**1.** To do this you just need to filter to get the `Center for Mathematics & Computing` rows and use the `energyKWH` variable for the response.
```{r Q7}
...
#### *Answer:*
**1.** Now we need to remove the CMC filter and *add* `building` to the grouping variable list (otherwise we will get average daily usage across all buildings on campus). It is hard to see any trends at all in this graph because of the large gap between the highest line (likely the "main campus" energy meter). Even without this meter's readings, we can't easily distinguish between buildings.   
...
**1.** There are `r nrow(EnergyBuildings)` buildings in the energy data

**2.** `r nrow(semi_join(EnergyBuildings,AAbuildings, by=c("building")))` of these were identified as acad/admin buildings via a join with the `AAbuildings` data. 

**3.** There are `r nrow(anti_join(AAbuildings,energy_narrow,by=c("building")))` buildings in the `AAbuildings` dataset that are not in the energy data. 
...
**3.** Plot the mean energy use per square foot by day of the week for the top 5 buildings found the the previous question and include info about their year built. Compare the daily trends in these buildings. 

#### *Answer:* 
**1.** With no missing data, there should be `r 4*24*31` readings in a month for each building. We are averaging these to get our top energy users in Oct. (average is better than sum since some buildings may have energy meters that were offline for some reason, creating the possibility of missing data.) The top 5 energy users were Weitz, Laird, AGH/Johnson, Gould, Sayles-Hill. 
```{r Q11a}
energyJoin %>% 
  filter(month==10) %>% 
  group_by(building) %>% 
...
**2.** We just need to divide by `sqFt` to get the usage reading adjusted for the size of a building (KWH/sqft). The top 5 users by square footage are Laird , AGH/Johnson, Hoppin House, Strong House and Sayles-Hill.
```{r Q11b}
top5 <- energyJoin %>% 
  filter(month==10) %>% 
  group_by(building) %>%
...
**3.** The `top5` dataset created for part b gives us the names we want to pull from the joined data `energyJoin`. We use `inner_join` to do a filtering join. After the join, we then need to filter to get October (we need to do this again, `top5` only gives building names, we still need to filter the joined energy data again), then group by building *and* day of the week and compute average usage per square foot. We see relatively flat trends for most buildings while Laird drops on the weekend. 

This makes sense if Laird Hall is an academic building while the others are residential. 
