...
The goal of today's activity is to examine the tradeoff between goodness of fit and over-fitting. We will compare the `training MSE` and the `testing MSE`
...
> The 10th degree polynomial fits the data much better. It might even be "too good" in the sense that it is over-fitting the model if we want to use it to predict out of sample.
```{r}
...
The `MSE` is declining as flexibility increases. In other words, as you increase the model flexibility, the model is able to fit the training data better.
...
> The testing `MSE` actually **INCREASES** as flexibility increases in contrast to the training `MSE`. This shows the trade-off between goodness of fit and over-fitting. 
